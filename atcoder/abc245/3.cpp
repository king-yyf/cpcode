...
void solve(){
	...
	for(int i=2;i<=n;++i){
		if(c[i-1]==1){
			if(abs(a[i]-a[i-1])<=k) c[i]+=1;
			if(abs(b[i]-a[i-1])<=k) c[i]+=2;
		}else if(c[i-1]==2){
			if(abs(a[i]-b[i-1])<=k) c[i]+=1;
			if(abs(b[i]-b[i-1])<=k) c[i]+=2;
		}else if(c[i-1]==3){
			if(abs(a[i]-b[i-1])<=k || abs(a[i]-a[i-1])<=k) c[i]+=1;
			if(abs(b[i]-b[i-1])<=k || abs(b[i]-a[i-1])<=k) c[i]+=2;
		}
		if (c[i] == 0) {
			wt("No\n");
			return;
