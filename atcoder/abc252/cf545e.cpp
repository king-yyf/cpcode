    ...
    while (pq.size()) {
        ...
        for (auto &v : g[t]) {
            ...
            if (dis[t] + cost < dis[x]) {
                ...
                ansEdge[x] = v;
                pq.push({-dis[x], x});
            } else if (dis[t] + cost == dis[x] && edges[ansEdge[x]][2] > edges[v][2]) {
            	ansEdge[x] = v;
            } 
        }
    }
...
void solve(){
	...
	long long c=0;
	cin>>x;
	auto a=shortestPathTree(n,e,x-1);

	f0(n)if(i!=x-1)c+=e[a[i]][2];
	cout<<c<<"\n";
	f0(n)if(i!=x-1)cout<<a[i]+1<<" ";
	cout<<"\n";
}
int main(){
