...
mint get(mint a1, mint d, mint n) {
    return n * a1 + n * (n - 1) * d / 2;
}

mint rangeOddSum(long long x1, long long x2, long long y1, long long y2) {
    long long col = y2 - y1 + 1, row = x2 - x1 + 1;
    long long n1 = (x2 - x1 + 2) / 2, n2 = (x2 - x1 + 1) - n1;
    int d = (x1 + y1) % 2;

    long long start1 = (x1 - 1) * m + y1, start2 = start1 + m;
    long long cnt1 = col / 2, cnt2 = col / 2;
    if (d == 0) {
        start1++;
        if (col % 2 == 1) cnt2++;
    } else {
    ...
    mint s1 = get(start1, 2, cnt1);
    mint s2 = get(start2, 2, cnt2);

    mint s3 = get(s1, cnt1 * 2 * m, n1);
    mint s4 = get(s2, cnt2 * 2 * m, n2);
    return s3 + s4;
}

mint solve(long long x1, long long x2, long long y1, long long y2) {
    mint s = 0;
    mint leftup = (x1 - 1) * m + y1, len = y2 - y1 + 1;
    mint s1 = get(leftup, 1, len);
    s = get(s1, m * len, x2 - x1 + 1);
    mint s2 = rangeOddSum(x1, x2, y1, y2);
    s = s - s2;
    return s;
}
