...
void ac_yyf(int tt) {
    ...
    for(int i=0;i<min(3,sz(a));++i){
    	for(auto &[x,y]:b){
	    	s1=a[i].second+y;
	    	if(mp.count({a[i].second,x})) s1-=mp[{a[i].first,x}];
	    	cmx(mx,s1);
	    }
    }

    for(int i=0;i<min(3,sz(b));++i){
    	for(auto &[x,y]:a){
	    	s1=b[i].second+y;
	    	if(mp.count({x,b[i].first})) s1-=mp[{x,b[i].first}];
	    	cmx(mx,s1);
	    }
    }
