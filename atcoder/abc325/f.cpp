...
void ac_yyf(int tt) {
    ...
    rd(l1,c1,k1,l2,c2,k2);

    ll mx=k1*l1+k2*l2,sm=Sm(d);
    ...
    ndarr<ll,3> f({n+1,k1+1,k2+1},-1);

    f[{0,0,0}]=1;

    function<ll(int, int, int)> dfs = [&](int p, int x1, int x2)->ll {
    	if(p>=n && x1>=0&&x2>=0)return 0;
    	if(f[{p,x1,x2}]!=-1)
    		return f[{p,x1,x2}];
    	ll res=1e16;
    	ll nd=d[p];
    	for(int i=0;i<=x1;++i){
    		int x=(nd-i*l1+l2-1)/l2;
    		if(x>x2)continue;
    		if(x<0)x=0;
    		ll cost=i*1ll*c1+x*1ll*c2;
    		res=min(res,cost+dfs(p+1,x1-i,x2-x));
    	}
    	if(res<1e16)
    	return f[{p,x1,x2}]=res;
    	else return f[{p,x1,x2}]=-1;
    };

    dfs(0,k1,k2);
    if(f[{0,k1,k2}]==-1 || f[{0,k1,k2}]>1e15){
    	wt(-1,nl);
    	return;
    }
    ...
    // f0(n){
    // 	for(int j=0;j<=k1;++j){
    // 		int x=(d[i]-j*l1+l2-1)/l2;
    // 		if(x>k2)continue;
    // 		if(x<0)x=0;
    // 		f[{i+1,j,x}]=min(f[{i+1,j,x}], )
    // 	}
    // }
