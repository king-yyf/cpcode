...
void ac_yyf(int tt) {
    ...
    f0(n)f2(j,m)b[i][j]=t++;
    // f0(n)wt(b[i]);wt(nl);
    auto get=[&](vector<vector<int>>& d){
    	string s(n*m,' ');
    	int t=0;
    	f0(n)f2(j,m)s[t++]=char(d[i][j]);
		string_hash f(s);
		return f.complete_hash();
    };
    ...
    map<uint64_t,vector<vector<int>>> mp;
    mp[xx]=a;

    auto rot=[&](vector<vector<int>> &c, int x, int y){
    	auto ans=c;
    	for(int i=x;i<x+n-1;++i){
    		for(int j=y;j<y+m-1;++j){
    			ans[i][j]=c[x+n-2-i+x][y+m-2-j+y];
    		}
    	}
    	return ans;
    };
    ...
    q=queue<pair<uint64_t,int>>();
    q.push({tar,0});
    map<uint64_t,vector<vector<int>>> m2;
    m2[tar]=b;
    map<uint64_t, int> d2;
    d2[tar]=0;
