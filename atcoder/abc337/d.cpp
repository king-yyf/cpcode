...
void ac_yyf(int tt) {
    ...
    vector q(n,vector<int>(m));
    f0(n)f2(j,m){
    	if(a[i][j]=='x')q[i][j]=1;
    	else if(a[i][j]=='o')p[i][j]=1;
    }
    PrefixSum2D<int> f(p),g(q);
    int c=1e9;
    f0(n){
    	for(int j=0;j+k<=m;++j){
    		if(g.sum(i,j,i,j+k-1)==0){
    			cmn(c,k-f.sum(i,j,i,j+k-1));
    		}
    	}
    }
    // wt(c,nl);
    ...
    f2(j,m){
    	for(int i=0;i+k<=n;++i){
    		if(g.sum(i,j,i+k-1,j)==0){
    			cmn(c,k-f.sum(i,j,i+k-1,j));
    		}
    	}
    }
    if(c>1e8)c=-1;
