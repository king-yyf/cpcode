...
void solve(){
	...
	for(int i=1;i<n;++i){
		for(int j=i+1;j<=n;++j){
			...
			{
				mp[a[i]+a[j]]=2;
				mp[a[i]+a[j]]=max(2,mp[a[i]]+mp[a[j]]);
			}
		}
	}
	for(int i=1;i<n-1;++i){
		...
		for(int j=i+1;j<n;++j){
			...
			for(int k=j+1;k<=n;++k){
				...
				{
					mp[x]=3;
					// if(mp[a[i]]+mp[a[j]]+mp[a[k]]>3)mp[x]=4;
					// mp[x]=max(mp[x],mp[a[i]]+mp[a[j]]+mp[a[k]]);
					// mp[x]=max(mp[x],mp[a[i]]+mp[a[j]+a[k]]);
					// mp[x]=max(mp[x],mp[a[j]]+mp[a[i]+a[k]]);
					// mp[x]=max(mp[x],mp[a[k]]+mp[a[j]+a[i]]);
				}
			}
		}
	}
