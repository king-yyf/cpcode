...
void solve(){
	...
	f0(n)cin>>a[i]>>b[i]>>c[i];
	vector<int> f(n,0),g(n,0),h(n,0);
	f[0]=a[0],g[0]=b[0],h[0]=c[0];
	for(int i=1;i<n;++i){
		f[i]=max(f[i],max({g[i-1],h[i-1]})+a[i]);
		g[i]=max(g[i],max({f[i-1],h[i-1]})+b[i]);
		h[i]=max(h[i],max({f[i-1],g[i-1]})+c[i]);
	}
	cout<<max({f[n-1],g[n-1],h[n-1]})<<"\n";
}
int main(){
