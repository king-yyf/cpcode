...
struct BFSTree {
    ...
    bool is_build;
    vector<vector<int>> g;
    vector<int> dep, seq, ids,fa, h; //h[i]:高度为i的节点起始编号
    BFSTree(){init(0);}
    ...
    void init(int _n) {
        ...
        dep.resize(n); g.resize(n);fa.resize(n);
    }
...
void ac_yyf(int tt) {
    ...
    f0(k){
    	...
    	x--;
    	int d1=g.dep[x];
    	if(d1==g.h.size()-1)continue;

    	d1++;

    	// dbg("d1=",d1,nl);

    	int l1=g.h[d1-1]+1,r1=g.h[d1];
    	int l,r;
