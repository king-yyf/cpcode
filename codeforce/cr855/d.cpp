...
void ac_yyf(int tt) {
    ...
    set<uint64_t> st;
    for(int i=0;i+2<=n;++i){
    	if(i==0) st.insert(f.substring_hash(2,n));
    	else if(i==n-2)st.insert(f.substring_hash(0,n-2));
    	else {
    		auto h1=f.substring_hash(0,i);
    		auto h2=f.substring_hash(i+2,n);
    		auto h=concat_hashes(h1,h2,n-(i+2));
    		st.insert(h);
    	}
    }
    wt(sz(st),'\n');
