...
int main() {
    ...
    sort(A.begin(), A.end());
    sieve(A.back());
    vector<mod_int> knapsack(N + 1, 0);
    knapsack[0] = 1;
    mod_int constant = factorial(N);
    ...
    for (int i = 0, j = 0; i < 2 * N; i = j) {
        ...
        constant *= inv_factorial(j - i);
        if (prime[A[i]]) {
            cout<<"p= "<<A[i]<<", "<<knapsack[N]<<"\n";
            for (int x = N - 1; x >= 0; x--)
                knapsack[x + 1] += knapsack[x] * (j - i);
        }
    }
    cout<<knapsack[N]<<"\n";
    cout << constant * knapsack[N] << '\n';
}
