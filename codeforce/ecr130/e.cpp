...
void solve(){
	...
	f0(n){
		for(int j=i+1;j<n;++j){
			...
			for(int k=0;k<n;++k){
				...
				long long d1=dis(a[i],a[k]),d2=dis(a[j],a[k]);
				if(d1<=d||d2<=d){
					o=0;break;
				}
			}
	...
	for(ll y=1;y<=x;++y){
		ans+=comb(x,y)*comb(n,y)*s[y]*comb(n-y,n-2*y) * s[n-2*y];
	}
	...
	for(int i=0;i<n;++i){
		for(int j=i+1;j<n;++j){
			for(int k=j+1;k<n;++k){
				long long x=dis(a[i],a[j]),y=dis(a[j],a[k]),z=dis(a[k],a[i]);
				if(x==y&&x==z&&y==z){
					mp[x].insert(x);
					mp[x].insert(y);
					mp[x].insert(z);
	...
	for(long long y=1;y<=c;++y){
		ans+=comb(c,y)*comb(n,y)*s[y]*comb(n-y,n-3*y)*s[n-3*y];
	}
	dbg("ans3",ans,c,'\n');
