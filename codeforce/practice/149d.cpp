...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
    	if (st[i] == '(') sk.push(i);
    	else cp[sk.top()] = i, sk.pop();
    }

    auto chk = [&](int x, int y) {
    	return min(x, y) == 0 && max(x, y) != 0;
    };

    function<mint(int, int, int, int)> dp = [&](int l, int r, int s, int t) {
    	if (vis[l][r][s][t]) return f[l][r][s][t];
    	vis[l][r][s][t] = true;
    	if (l > r) return f[l][r][s][t] = (!s && !t);
    	if (cp[l] == r) {
    		for (int x = 0; x <= 2; ++x) 
    			for (int y = 0; y <= 2; ++y) 
    				if (chk(s, t) && !(s & x) && !(y & t)) 
    					f[l][r][s][t] += dp(l + 1, r - 1, x, y);
    	} else {
    		for (int x = 0; x <= 2; ++x)
    			for (int y = 0; y <= 2; ++y){
    				if (chk(s, x) && !(x & y)) 
    					f[l][r][s][t] += dp(l, cp[l], s, x) * dp(cp[l] + 1, r, y, t);
    			}
    	}
    	return f[l][r][s][t];
    };
