...
int main(){
	...
	cin>>n>>s;
	long long ans=0;
	string_hash<string> f(s); //forward
	// reverse(s.begin(),s.end());
	// string_hash<string> g(s); //forward

    auto cal=[&](int x,int y,int len){
        int l=1,r=len,mid,ans=0;
        while(l<=r){
            mid=(l+r)/2;
            if(f.substring_hash(x-1,x+mid)==f.reverse_substring_hash(y-mid-1,y)) l=mid+1;
	...
	cout<<"n="<<n<<"\n";
	for(int i=1;i<=n;++i){
        if(s.substring_hash(0,i)==s.reverse_substring_hash(1,i+1)){
            if(i&1)ans+=26;
            else ans++;
            continue;
        }
