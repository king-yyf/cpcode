...
void ac_yyf(int tt) {
    ...
    vector<vector<mint>> f(n+1,vector<mint>(s+1));
    ...
    for(int i=0;i<n;++i){
        for(int j=0;j<=s;++j){
            if(j<a[i]){
                ...
                f[i+1][j]=f[i][j]+f[i][j-a[i]];
            }
        }
    }


    // vector<vector<int>> p(n+1,vector<int>(s+1,s+10));
    ...
    //             p[i][j]=j;
    //         }else{
    //             if(j<s)p[i][j]=p[i][j+1];
    //         }
    //     }
    // }

    vector<vector<int>> f1(n + 1, vector<int>(s + 1));
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 0; j <= s; ++j) {
            f1[i][j] = f1[i - 1][j];
            if (j >= a[i - 1]) f1[i][j] = (f1[i][j] + f1[i - 1][j - a[i - 1]]) % 998244353;
        }
    }


    // f0(n+1)f2(j,s+1){
    //     assert(f1[i][j]==f[i][j].val());
    // }
