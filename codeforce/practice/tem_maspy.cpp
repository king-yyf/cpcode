...
#line 2 "/home/maspy/compro/library/linalg/bitset/solve_linear.hpp"
...
初期状態の R 成分 → 現在の R,Y 成分
初期状態の B 成分 → 現在の R,Y 成分
*/
...
void solve() {
  ...
  FOR(K) {
    ...
    if (S == "mix") {
      ...
      FOR(n) {
        INT(i);
        --i;
        if (R[i] == 'R' || R[i] == 'B') A[2 * i + 0] = 1;
        if (R[i] == 'Y' || R[i] == 'B') B[2 * i + 0] = 1;
        if (Y[i] == 'R' || Y[i] == 'B') A[2 * i + 1] = 1;
        if (Y[i] == 'Y' || Y[i] == 'B') B[2 * i + 1] = 1;
      }
      mat.eb(A);
      mat.eb(B);
      STR(x);
    ...
    auto change = [&](char a, char b) -> void {
      INT(n);
      FOR(n) {
        INT(i);
        --i;
        if (R[i] == a || R[i] == b) R[i] ^= a ^ b;
        if (Y[i] == a || Y[i] == b) Y[i] ^= a ^ b;
      }
    };
    if (S == "RY") { change('R', 'Y'); }
    if (S == "RB") { change('R', 'B'); }
  ...
  auto X = solve_linear<BS, bool>(len(mat), 2 * N, mat, rhs);
  ...
  string ANS;
  FOR(i, N) {
    if (!x[2 * i + 0] && !x[2 * i + 1]) ANS += ".";
    if (!x[2 * i + 0] && x[2 * i + 1]) ANS += "Y";
    if (x[2 * i + 0] && !x[2 * i + 1]) ANS += "R";
    if (x[2 * i + 0] && x[2 * i + 1]) ANS += "B";
  }
  print(ANS);
}
