...
struct SA {
    ...
    // bool comp(int a, int b, int len = -1) const { 
    //     if (len < 0) len = n;
    //     if (a == b) return 0;
...
int main() {
    ...
    sort(all(v),
         [&st](const pair<int, pair<int, int>> & a, const pair<int, pair<int, int>> & b) -> bool
    {
        int p1 = a.first, p2 = b.first;
        int len1 = a.second.second - a.second.first + 1;
        int len2 = b.second.second - b.second.first + 1;
