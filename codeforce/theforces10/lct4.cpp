...
F merge(F x, F y) { return min(x, y); }
F id() { return inf; }
...
public:
    int minimumVisitedCells(vector<vector<int>>& a) {
        ...
        LazySegTree<S, op, e, F, tag, merge, id> s1(n * m), s2(n * m);
        s1.set(0, 1); s2.set(0, 1);
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < m; ++j) {
                int t = min(s1.get(j * n + i), s2.get(i * m + j));
                if (t == inf) continue;
                s1.apply(j * n + i + 1, j * n + min(i + a[i][j], n - 1) + 1, t + 1);
                s2.apply(i * m + j + 1, i * m + min(j + a[i][j], m - 1) + 1, t + 1);
            }
        }
        int ans = min(s1.get(m * n - 1), s2.get(m * n - 1));
