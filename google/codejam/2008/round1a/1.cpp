...
void solve(){
	cin>>n;
	vector<long long > x1,x2,y1,y2,v1,v2;
	f0(n){
		cin>>x;
	...
	m=min(sz(x1),sz(y2));
	f0(m) ans+=x1[i]*y2[i];
	if(sz(x1)>m) for(int i=m;i<sz(x1);++i) v1.push_back(x1[i]);
	else for(int i=m;i<sz(y2);++i) v1.push_back(y2[i]);

	sor(y1);sort(rall(x2));
	m=min(sz(y1),sz(x2));
	f0(m) ans+=y1[i]*x2[i];
	if(sz(y1)>m) for(int i=m;i<sz(y1);++i) v2.push_back(y1[i]);
	else for(int i=m;i<sz(x2);++i) v2.push_back(x2[i]);

	sort(all(v1));
	sort(rall(v2));
	f0(sz(v1)) ans+=v1[i]*v2[i];
	wt(ans,'\n');
}
int main(){
