...
public:
    ...
    string alienOrder(vector<string>& words) {
    	...
    	for(auto& s : words) {
     	...
     	vector<pair<char,int>> indeg;
     	for (int i = 0; i < 26; ++i) {
     		if (cnt[i]) {
     			table['a' + i] = indeg.size();
     			indeg.emplace_back('a'+i, 0);
     		}
     	} 
     	...
     	queue<char> q;
     	for(auto &it: indeg) {
     		if (it.second == 0) q.emplace(it.first);
     	}
     	string ans = "";
     	while (!q.empty()) {
     		for (auto &it: graph[q.front()]) {
     			if (-- indeg[table[it].second] == 0) {
     				q.emplace(it);
     			}
     		}
     		ans += q.front();
