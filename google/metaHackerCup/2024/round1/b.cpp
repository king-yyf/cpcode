...
int main() {
    freopen("/Users/yangyf/Downloads/prime_subtractorization_input.txt","r",stdin);
    freopen("/Users/yangyf/Desktop/cpcode/google/metaHackerCup/2024/round1/b_2.txt","w",stdout);
    int T = 1, fun_type = 0;
    n=sz(se.primes);

    c[5]=2;
    for(int i=6;i<N;++i) {
        if(se.is_prime(i)&&se.is_prime(i-2)){
            c[i]=c[i-1]+1;
        }else{
            c[i]=c[i-1];
        }
    ...
    // f0(n-1){
    //     if(se.primes[i+1]-se.primes[i]==2){
    //         if(!sz(pp) || pp.back()!=se.primes[i]){
    //             pp.push_back(se.primes[i]);
    //         }
    //         pp.push_back(se.primes[i+1]);
    //     }
    // }
    // // wt(pp);
    // m=sz(pp);
    ...
    for (int cas = 1; cas <= T; ++cas) {
        ...
        else if (fun_type == 1) cout <<"Case #" <<cas<<":"<< (boolf(cas) ? "Yes" : "No") << '\n';
        else cout <<"Case #" <<cas<<":"<< (boolf(cas) ? "YES" : "NO") << '\n';
    }
    return 0;
}
