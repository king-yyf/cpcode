...
long long pair_pop_count(vector<int> &a) {
    int n = a.size();
    vector<int> p(n);
    long long s = 0;
    vector<int> cnt(10);
    for (int i = 0; i < 10; ++i) {
        ...
        int d = (1 << i), u = 1 << (i + 1);
        for (int j = 1; j < n; ++j) {
            if (p[j] + p[j - 1] < d || p[j] + p[0] >= u) continue;
            int l = 0, r = j - 1, x1 = r, x2 = l;
            while (l <= r) {
                int md = (l + r) / 2;
                if (p[j] + p[md] >= d) {
                    x1 = md;
