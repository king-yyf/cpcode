...
void ac_yyf(int tt) {
    ...
    f0(m){
        ...
        if(g.on_path(a[0],a[1],a[2])) {
            int d=g.dist(a[1],a[2]);
            int c=d+1-3;
            c+=g.siz[a[2]]-1;
            c+=g.siz[a[1]]-1;
            wt(c,nl);
        ...
        if(g.on_path(a[1],a[0],a[2])) {
            int c=0;
            if(g.is_anc(a[0], a[1])) {
                int d=g.kth_node_on_path(a[0],a[1],1);
                c=n-1-g.siz[d];
                c+=(g.siz[a[2]]-1);
                c+=g.dist(a[0],a[2])-2;
            }else{
                int d=g.dist(a[0],a[2]);
                c=d+1-3;
                c+=g.siz[a[2]]-1;
                c+=g.siz[a[0]]-1;
            }
        ...
        // if(g.on_path(a[0],a[1],a[2])) {
        //     int c=g.siz[a[1]]+g.siz[a[2]]-2;
        //     wt(c,nl);
        //     continue;
        // }

        // if(g.on_path(a[0],a[1],a[2])) {
        //     int c=g.siz[a[0]]+g.siz[a[2]]-2;
        //     wt(c,nl);
        //     continue;
        // }
