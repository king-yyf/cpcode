...
void ac_yyf(int tt) {
    ...
    f0(q){
    	rd(x,y);
    	auto p=seg.get(x-1,y);
    	ll c=0;
    	f2(j,K){
    		ll one=p[j],zero=y-x+1-p[j];
    		ll c1=zero*(zero-1)/2*one;
    		ll c2=one*(one-1)*(one-2)/6;
    		// if(j<3){
    		// 	dbg(one,zero,c1,c2,nl);
    		// }
    		c=(c+((c1+c2)%mod) * (1ll<<j))%mod;
    	}
        wt(c,nl);
    }
}
