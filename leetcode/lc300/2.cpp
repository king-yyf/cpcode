...
public:
    vector<vector<int>> spiralMatrix(int m, int n, ListNode* h) {
        ...
        while(sz(v)<m*n) v.push_back(-1);
        int u = 0, d = m - 1, l = 0, r = n - 1, p = 0;
        vector<vector<int>> res(m,vector<int> (n)) mx;
        while (u <= d && l <= r) {
            for (int i = l; i <= r; ++i) mx[u][i]= v[p++];
            if (++u > d) break;
            for (int i = u; i <= d; ++i)  mx[i][r]= v[p++];
            if (--r < l) break;
            for (int i = r; i >= l; --i)  mx[d][i]= v[p++];
            if (--d < u) break;
            for (int i = d; i >= u; --i) mx[i][l]= v[p++];
            ++l;
        }
        return mx;
    }
