...
struct weighted_LCA {
    ...
    void add_edge(int a, int b, T_weight weight, T_weight weight2) {
        adj[a].emplace_back(b, weight2);
...
public:
    long long maxOutput(int n, vector<vector<int>>& es, vector<int>& p) {
        weighted_LCA<long long> g(n * 2);
        for (auto &e: es) {
        	int u = e[0], v = e[1];
        	g.add_edge(u, v, p[u], p[v]);
        }
        for (int i = 0; i < n; ++i) {
            g.add_edge(i, i + n, 0, p[i]);
        }
        g.build();
        auto diameter = g.get_diameter();
        int u = diameter.second[0], v = diameter.second[1];
        u%=n,v%=n;
        cout << u << " , " << v << "\n";
        long long ans = 0;
        for (int i = 0; i < n; ++i) {
            cout << " i= "<<i<<", "<< g.get_weighted_dist(i+n, u) <<", " <<  g.get_weighted_dist(i+n, v) << ", "<<p[i]<<" "<<p[u]<<" "<<p[v]<<"\n"; 
            ans = max({ans, g.get_weighted_dist(i, u) - p[i], g.get_weighted_dist(i, v) -  p[i]});
        }
        return ans;
    }
};
...
int main() {
    ...
    Solution so;
    vector<vector<int>> es={{0,1},{1,2},{1,3},{3,4},{3,5}};
     // vector<vector<int>> es={{0,1},{1,2}};
    vector<int> p={9,8,7,6,10,5};
     // vector<int> p{1,1,1};
    char ch='华';
    cout<<ch<<"\n";
    int n=6;
    auto ans =so.maxOutput(n,es,p);
    cout<<ans<<"\n";
}