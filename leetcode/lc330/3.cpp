...
// https://codeforces.com/gym/102220/submission/74281152
// https://www.cnblogs.com/clrs97/p/11080565.html

class Solution {
public:
    long long countQuadruplets(vector<int>& a) {
        int n=sz(a),mx=Max(a);
        vector f(mx+1,vector<int>(mx+1));
        f0(n){
            f[i+1][a[i]]=1;
        }
        PrefixSum2D<long long> p(f);
        long long c=0;
        for(int j=1;j<n;++j)for(int k=j+1;k<n;++k){
            if(a[j]>a[k]){
                c+=p.sum(0,0,j-1,a[k-1])*p.sum(k+1,a[j]+1,mx,mx);
            }
        }
        return c;
    }
