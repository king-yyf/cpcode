...
class Solution {
public:
    vector<int> maximumSumQueries(vector<int>& v1, vector<int>& v2, vector<vector<int>>& qs) {
        n=sz(v1),m=sz(qs);
        vector<ar3> a(n);
        f0(n)a[i]={v1[i],v2[i],v1[i]+v2[i]};
        vector<ar3> q(m);
        f0(m){
        	q[i]={qs[i][0],qs[i][1],i};
        }
        ...
        int j=0;
        f0(m){
        	while(j<n&&a[j][0]<q[i][0]){
        		seg.set(mp[j],-1);
        		j++;
        	}
        	if(j>=n)continue;
        	if(b[n-1][0]<q[i][1])continue;
        	int l=0,r=n-1,x=n-1;
        	while(l<=r){
        		int md=(l+r)/2;
        		if(b[md][0]>=q[i][1]){
