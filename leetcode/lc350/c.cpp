...
public:
    int specialPerm(vector<int>& a) {
    	n=sz(a);
    	vector<vector<mint> > f((1<<n)+2,vector<mint>(16,0));
    	vector<vector<int>> vis((1<<n)+2,vector<int>(16));
    	function<mint(int, int,int)> dfs = [&](int x, int i,int msk) {
    		if(i==n)return 1;
    		if(vis[x+1][msk])return f[x+1][msk];
    		mint c=0;
    		f2(j,n){
    			if(msk&(1<<j)){
    				continue;
    			}
    			if(x==-1||a[x]%a[j]==0||a[j]%a[x]==0){
    				c+=dfs(j,i+1,msk|(1<<j));
    			}
    		}
    		return f[x+1][msk]=c;
    	};
    	return dfs(-1,0,0).val();
