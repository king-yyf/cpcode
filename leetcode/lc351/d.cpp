...
class Solution {
public:
    vector<int> survivedRobotsHealths(vector<int>& ps, vector<int>& hs, string s) {
        ...
        f0(n){
        	if(a[i][2]==0) {
        		if(sk.empty()||a[sk.top()][2]==0) sk.push(i);
        		else{
        			while(sz(sk)&&a[i][1]>a[sk.top()][1]){
        				sk.pop();
        				a[i][1]--;
        			}
        			if(sz(sk)&&a[i][1]==a[sk.top()][1]){
        				sk.pop();
        				continue;
        			}
        			if(sz(sk)&&a[i][1]<a[sk.top()][1]){
        ...
        while(sz(sk)){
        	c.push_back(sk.top());sk.pop();
        }
        if(sz(c)==0)return {};
        sort(all(c),[&](auto &x,auto &y){return a[x][3]<a[y][3];});
        f0(sz(c))c[i]=a[c[i]][1];
        return c;
    }
};
