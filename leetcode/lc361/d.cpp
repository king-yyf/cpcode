...
class Solution {
public:
    vector<int> minOperationsQueries(int n, vector<vector<int>>& es, vector<vector<int>>& qs) {
        HLD g(n);
        vector<vector<pair<int,int>>>  g1(n);
        for(auto&v:es){
        	g.add_edge(v[0],v[1]);
        	g1[v[0]].emplace_back(v[1],v[2]);
        	g1[v[1]].emplace_back(v[0],v[2]);
        }
        g.build();
        ...
        f0(m){
        	...
        	int mx=0;
        	f2(j,26){
        		cmx(mx,f[x][i]+f[y][i]-2*f[g.lca(x,y)][i]);
        	}
        }
        return c;
    }
