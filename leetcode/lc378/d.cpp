...
class Solution {
public:
    vector<bool> canMakePalindromeQueries(string S, vector<vector<int>>& qs) {
        int m=sz(qs);
        vector<bool>ans(m);
        string s=S.substr(0,sz(S)/2);
        string t=S.substr(sz(S)/2);
        rev(t);
        int n=sz(s);
        string_hash f(s);
        string_hash g(t);

        FenwickTree2D<int> s1(n,26),s2(n,26);
        f0(n)s1.add(i,s[i]-'a',1);
        f0(n)s2.add(i,t[i]-'a',1);
        
        cout<<s<<"\n";
        cout<<t<<'\n';
        
        f0(m){
        	int a=qs[i][0],b=qs[i][1],c=qs[i][2]-n,d=qs[i][3]-n;
        	c=n-1-c,d=n-1-d;swap(c,d);
        	int l=min(a,c),r=max(b,d);
            cout<<a<<", "<<b<<", "<<c<<", "<<d<<", "<<l<<", "<<r<<"\n";
        	if(l>0&&f.substring_hash(0,l)!=g.substring_hash(0,l)){
        		continue;
        	}
        	if(r<n-1&&f.substring_hash(r+1,n)!=g.substring_hash(r+1,n)){
        		continue;
        	}
        	if(min(b,d)>=max(a,c)) {
        		if((a<=c&&b>=d)||(c<=a&&d<=b)){
        			bool o=1;
	        		f2(j,26){
	        			if(s1.ask(l,j,r,j)!=s2.ask(l,j,r,j)){
	        				o=0;break;
	        			}
	        		}
	                // cout<<"i="<<i<<", o="<<o<<"\n";
        		...
        		if(b<c) {
        			if(f.substring_hash(b+1,c)!=g.substring_hash(b+1,c)) continue;
        			bool o=1;
	        		f2(j,26){
	        			if(s1.ask(a,j,b,j)!=s2.ask(a,j,b,j)){
	        				o=0;break;
	        			}
	        		}
	        		f2(j,26){
	        			if(s1.ask(c,j,d,j)!=s2.ask(c,j,d,j)){
	        				o=0;break;
	        			}
	        		}
	        		if(o){
        			...
        			if(f.substring_hash(d+1,a)!=g.substring_hash(d+1,a)) continue;
        			bool o=1;
	        		f2(j,26){
	        			if(s1.ask(a,j,b,j)!=s2.ask(a,j,b,j)){
	        				o=0;break;
