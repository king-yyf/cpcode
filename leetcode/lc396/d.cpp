...
class Solution {
public:
    int minCostToEqualizeArray(vector<int>& a, int x, int y) {
    	sort(all(a));
    	mint c=0;
    	n=sz(a);
    	if(n==2){
        ...
        if(x*2<=y){
        	...
        	ll s3=0;
        	f1(n-1)s3+=a[i];
        	auto get=[&](int mx)->long long{
        		ll s1=mx-a[0],s2=mx*1ll*(n-1)-s3;
        		// f1(n-1){
        		// 	s2+=mx-a[i];
        		// }
        		ll s=0;
        	...
        	ll c1=1e18;
        	for(int i=a[n-1];i<a[n-1]+100;++i){
        		cmn(c1,get(i));
        		// dbg("i=",i,"cur=",get(i),nl);
        	}
        	c=c1;
        }
        return c.val();
...
#ifdef sigma-yyf
int main(){
    vector<int> a{5,5,7,8};
    Solution so;
    auto o=so.minCostToEqualizeArray(a,7,5);
    wt(o,nl);
