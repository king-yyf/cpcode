...
public:
    int maxScore(vector<vector<int>>& g) {
        ...
        f[m-1][n-1] = g[m-1][n-1];
        r3(j,n-1,0)f[m-1][j] = max(f[m-1][j+1], g[m-1][j]);
        r3(i,m-1,0)f[i][n-1] = max(f[i+1][n-1], g[i][n-1]);
        r3(i,m-1,0) {
           r3(j,n-1,0) {
                f[i][j]=max(g[i][j],max(f[i+1][j], f[i][j+1]));
            }
        }

        int c = -inf;
        f0(m)f2(j,n){
            if(j+1<n)cmx(c,f[i][j+1] - g[i][j]);
            if(i+1<m)cmx(c,f[i+1][j] - g[i][j])
        }
