...
public:
    long long maxScore(vector<int>& a, int x) {
        ...
        if(a[0]&1)s1.set(0,a[0]);
        else s2.set(0,a[0]);
        for(int i=1;i<n;++i){
        	if(a[i]&1){
        		ll mx1=s1.get(0,i),mx2=s2.get(0,i);
        		f[i+1]=max(mx1+a[i],mx2+a[i]-x);
        		s1.set(i,f[i+1]);
        	}else{
        		ll mx1=s1.get(0,i),mx2=s2.get(0,i);
        		f[i+1]=max(mx1+a[i]-x,mx2+a[i]);
        		s2.set(i,f[i+1]);
        	}
        }
        wt(f);
...
#ifdef sigma-yyf
int main(){
    vector<int> a{2,3,6,1,9,2};int x=5;
    Solution so;
    auto p=so.maxScore(a,x);
    cout<<'\n';
    return 0;
}
#endif
