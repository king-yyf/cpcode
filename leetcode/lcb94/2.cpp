...
class Solution {
public:
    vector<int> topStudents(vector<string>& ps, vector<string>& ns, vector<string>& a, vector<int>& b, int k) {
        set<string> s1,s2;
        for(auto&x:ps)
            s1.insert(x);
        for(auto&x:ns)
        ...
        for(int i=0;i<sz(a);++i){
            auto pp=split(a[i],' ');
            for(auto&ppp:pp){
                if(s1.count(ppp))mp[b[i]]+=3;
                else if(s2.count(ppp)) mp[b[i]]-=1;
            }
