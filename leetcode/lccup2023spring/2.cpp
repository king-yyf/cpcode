...
class Solution {
public:
    int adventureCamp(vector<string>& a) {
        int n=sz(a);
        string s=a[0];
        set<uint64_t> st;
        auto p=split(s,'>');
...
int main(){
    ...
    vector<string> a{"leet->code","leet->code->Campsite->Leet","leet->code->leet->courier"};
    auto p=so.adventureCamp(a);
    cout<<p;
