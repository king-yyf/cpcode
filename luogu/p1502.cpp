...
void solve() {
    ...
    for (auto &[x, y, w]: a) {
    	int l, r, d, u;
    	long long c;
    	l = dx(x), d = dy(y), r = dx(x + w), u = dy(y + h);
    	if (r < nx && dx[r] < x + w) r++;
    	if (u < ny && dy[u] < y + h) u++;
    	c = segs[r].get(d, u) - segs[l].get(d, u);
    	ans = max(ans, c);

    	l = dx(x - w), d = dy(y), r = dx(x), u = dy(y + h);
    	if (u < ny && dy[u] < y + h) u++;
    	c = segs[r].get(d, u) - segs[l].get(d, u);
    	ans = max(ans, c);

    	r = dx(x), u = dy(y), l = dx(x - w), d = dy(y - h);
    	c = segs[r].get(d, u) - segs[l].get(d, u);
    	ans = max(ans, c);

    	l = dx(x), u = dy(y), r = dx(x + w), d = dy(y - h);
    	if (r < nx && dx[r] < x + w) r++;
    	c = segs[r].get(d, u) - segs[l].get(d, u);
    	ans = max(ans, c);
