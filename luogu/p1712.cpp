...
void ac_yyf(int tt) {
    ...
    LazySegTree<S, op, e, F, tag, merge, id> seg(vector<S>(M,{0,0, 1}));
    int ans = -1;
    for(int i=0,j=0;i<n;++i){
    	while(j < n && seg.get_all().mx < m) {
    		seg.apply(a[j][0], a[j][1] + 1, 1);
    		j++;
    	}
    	if (seg.get_all().mx >= m && (ans == -1 || ans > a[j - 1][2] - a[i][2])) {
    		ans = a[j - 1][2] - a[i][2];
    	}
    	seg.apply(a[i][0], a[i][1] + 1, -1);
    }
    cout << ans << '\n';
}
