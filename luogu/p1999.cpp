...
void solve(){
	...
	vector<int> f1,f2;
	for (int i = 0; i < n; ++i) {
		int p = lower_bound(f1.begin(), f1.end(),a[i]) - f1.begin();
		if (p == f1.size()) f1.push_back(a[i]);
		else f1[p] = a[i];
	}
	for (int i = n - 1; ~i; --i) {
		int p = upper_bound(f2.begin(), f2.end(), a[i]) - f2.begin();
		if (p == f2.size()) f2.push_back(a[i]);
		else f2[p] = a[i];
	}
	cout << f2.size() << "\n" << f1.size() << "\n";
}
