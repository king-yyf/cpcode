...
int main() {
    ...
    for (int i = 0, op, l, r, x, y; i < q; ++i) {
    	...
    	if (op == 1) {
    		cin >> l >> r >> x;
    		cout << (x > mx ? 0 : segs[x].get(l - 1, r) ) << '\n';
    	} else {
    		cin >> x;
            int idx = x - 1;
            x = a[idx], y = a[idx + 1];
    		swap(a[idx], a[idx + 1]);
            segs[x] = segs[x].apply(idx, [](int e){return e - 1;});
            segs[x] = segs[x].apply(idx + 1, [](int e){return e + 1;});
            segs[y] = segs[y].apply(idx, [](int e){return e + 1;});
            segs[y] = segs[y].apply(idx + 1, [](int e){return e - 1;});
    	}
    }
