...
void ac_yyf(int tt) {
 ...
 17 -1 -1 
    ...
    */
    ...
    f0(n)b[i+1]+=b[i];f0(n)d[i+1]+=d[i];

    unordered_set<ll>s1(all(b)),s2(all(d)); 
    ll c=0;
    ...
    f0(n){
    	if(b[n]<b[i]+k)break;
    	if(d[n]<d[i]+k)break;
    	if(!s1.count(b[i]+k) && !s2.count(d[i]+k))continue;
    	...
    	// cmn(x,n);
    	if(l1<=n&&b[l1]!=b[i]+k&&d[l1]!=d[i]+k)continue;
