...
void ac_yyf(int tt){
    ...
    f0(n) {
        for(int j=1;j<=k;++j) {
            if(i<j-1||n-1-i<k-j)continue;
            c+=mint(10).pow(k-j)*a[i]*comb.C(i,j-1)*comb.C(n-1-i,k-j);
            // auto x1=comb.C(i,k-j),x2=comb.C(n-1-i,k-j);
            // auto x3=mint(10).pow(j-1)*a[i];
            // // dbg(i,j,a[i],x1,x2,x3);
        }
    }
    // for(int i=0;i+k<=n;++i){
    //     dbg(i,a[i],n-1-i,k);
    //     c+=mint(10).pow(k-1)*a[i]*comb.C(n-1-i,k-1);
    // }
    wt(c,nl);
}
int main() {
