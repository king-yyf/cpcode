...
void solve(int tt) {
    ...
    vector f(5005,vector<long long>(5005));

    f0(5005)f2(j,5005)f[i][j]=i^j;

    vector< string_hash<vector<long long>> > p(5005);

    f0(5005) {
        string_hash<vector<long long>> sh(f[i]);
    ...
    string_hash<vector<long long>> d(a);
    for(int i=0;i<n;++i){
        for(int j=i+1;j<n;++j){
            int l=j-i+1;
            bool o=0;
            for(int k=0;k<130;++k){
                for(int h=0;h+l<=130;++h){
                    if(p[k].substring_hash(h,h+l)==d.substring_hash(i,i+l)){
                        o=1;
                        break;
                    }
                }
