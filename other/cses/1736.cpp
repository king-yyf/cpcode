...
int main() {
    ...
    vector<S> a(n);
    for(int i=0;i<n;++i){
    	cin>>a[i].sum;a[i].size=1;
    } 
    LazySegTree<S, op, e, F, tag, merge, id> s2(a);
    for (int i = 0; i < n; ++i) {
        a[i].sum = 0, a[i].size = i+1;	
    }
    LazySegTree<S, op, e, F, tag, merge, id> s1(a);
    ...
    for (int i=0,t,x,y;i<q;++i){
    	...
    	if(t==1){
    		...
    		cout<<s1.get(x,y).sum+s2.get(x,y).sum<<'\n';
    	}
    }
