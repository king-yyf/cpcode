...
void ac_yyf(int tt) {
    ...
    LazySegTree<S, op, e, F, tag, merge, id> s(vector<S>(mx+1,{0,1}));
    ...
    for(int i=0,j=0;i<Q;++i){
        while(j<n&&a[j][0]<=qs[i][0]){
            q.push({a[j][1],a[j][0]});
            s.apply(a[j][0],a[j][1]+1,1);
            j++;
        }
        while(sz(q)&&q.top().first<qs[i][0]){
            s.apply(q.top().second,q.top().first+1,-1);
            q.pop();
        }
        ans[qs[i][2]]=sz(q)-s.get(qs[i][1]).sum;
    }
