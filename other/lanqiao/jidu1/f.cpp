...
// int get(int n,int k, int mn,int mx){
// 	if(n<mn)return 0;
// 	if(k==1)return 1;
// 	int s=0;
// 	for(int t=mn;t<=mx;++t){
...
int solve(int A, int B) {
    ...
    fact[0]=1;
    for(int i=1;i<=10*n+4;++i) fact[i]=(i*fact[i-1])%M;
    ll ans=0;
    for(int r=0;r<=n-1;++r){
        ll cc=C(n-1,r,fact);
        ll curpw=10*(n-1-r);
        if((n-1-r)%2) cc=M-cc;
        if(curpw<=s-1){
            ll val=C(s-1-curpw+n-1,n-1,fact);
            ans=(ans+(cc*val)%M)%M;
        } 
        curpw+=9;
        cc=M-cc;
