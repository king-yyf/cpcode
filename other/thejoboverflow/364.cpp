...
// http://www.thejoboverflow.com/problem/364/
...
void ac_yyf(int tt) {
    ...
    f0(n){
    	if(sz(g.g[i])==0){
    		int p=g.lca(i,s);
            int d1=g.dist(0,p),d2=g.dist(s,p);
            ll s1=0;
            if(d1<d2){
                g.path(0,i,[&](int u, int v){
                    s1+=ps[v]-ps[u];
                });
            }else{
                int d=g.dist(0,s);
                if(d%2==0){
                    int p2=g.kth_anc(s,d/2);
                    g.path(0,g.fa[p2],[&](int u, int v){
                        s1+=ps[v]-ps[u];
                    });
                    s1+=a[p2]/2;
                }else{
                    int p2=g.kth_anc(s,(d+1)/2);
                    g.path(0,p2,[&](int u, int v){
                        s1+=ps[v]-ps[u];
                    });
                }
                g.path(p,i,[&](int u, int v){
                    s1+=ps[v]-ps[u];
                });
                s1-=a[p];
            }
