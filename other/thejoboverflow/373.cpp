...
// http://www.thejoboverflow.com/problem/373/
...
vector<long long> leftMonsters(vector<vector<int>> &a, vector<vector<int>> &qs) {
	...
	long long tot = 0;
	vector<int> b;
	vector<vector<int>> p(mx + 1), f(mx + 1);
	for (int i = 0; i < n; ++i) {
	    p[a[i][0]].push_back(a[i][2]);
	    f[a[i][1]].push_back(a[i][2]);
	    tot += a[i][1] - a[i][0] + 1;
	    b.push_back(a[i][2]);
	}	
	vector<vector<pair<int,int>>> q(mx + 1);
	for (int i = 0; i < m; ++i) {
