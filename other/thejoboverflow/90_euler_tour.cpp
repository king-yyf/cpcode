...
vector<int> tree_maximum(vector<int> &a, vector<vector<int>> &es, vector<vector<int>> &qs){
	int n=sz(a);
	...
	for(auto&q:qs){
		if(q[0]==1){
			...
			ans.push_back(seg.get(et.in[q[1]],et.out[q[1]]));
		}
	}
	return ans;
}
